...
// // Utility: parse date into Date object for sorting (expects "DD Mon YYYY | HH:MM AM/PM")
// const parseDate = (str) => {
//     // Example: "12 Feb 2025 | 09:00 AM"
...
//                     totalValue: 54000000, // ₹ 5,40,00,000
//                     status: "Active",
//                 },
//                 {
//                     projectId: "PRJ-002",
...
//                     totalValue: 21050000, // ₹ 2,10,50,000
//                     status: "On Hold",
//                 },
//                 {
//                     projectId: "PRJ-004",
...
//     return (
//         <div className="min-h-screen bg-gradient-to-br from-[#003f9a] to-[#004bb8] px-4 py-6 flex justify-center">
//             <div className="w-full max-w-[1100px]">
//                 {/* Header */}
//                 <div className="mb-6 flex items-center justify-between">
//                     <div>
//                         <h1 className="text-white font-bold text-2xl tracking-wide">Available Contracts</h1>
//                         <p className="text-gray-200 text-sm mt-1">Select a project to proceed</p>
//                     </div>
//                     <div className="text-right">
//                         <p className="text-gray-200 text-xs">Total Value (visible page)</p>
...
//                             <label className="text-gray-200 text-xs">Search</label>
//                             <div className="mt-1 relative">
//                                 <input
//                                     value={search}
//                                     onChange={(e) => setSearch(e.target.value)}
//                                     placeholder="Search by Project ID, Company or Name"
//                                     className="w-full bg-white/5 text-white placeholder-gray-400 border border-white/15 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-300"
//                                 />
...
//                         <div>
//                             <label className="text-gray-200 text-xs">Company</label>
//                             <select
//                                 value={companyFilter}
//                                 onChange={(e) => setCompanyFilter(e.target.value)}
//                                 className="mt-1 w-full bg-white/5 text-white border border-white/15 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-300"
//                             >
//                                 {uniqueCompanies.map((c) => (
//                                     <option key={c} value={c} className="bg-[#0b2f6d] text-white">
//                                         {c}
//                                     </option>
//                                 ))}
//                             </select>
...
//                             <div>
//                                 <label className="text-gray-200 text-xs">Status</label>
//                                 <select
//                                     value={statusFilter}
...
//                                         <option key={s} value={s} className="bg-[#0b2f6d] text-white">
//                                             {s}
//                                         </option>
//                                     ))}
//                                 </select>
...
//                             <div>
//                                 <label className="text-gray-200 text-xs">Sort</label>
//                                 <select
//                                     value={sortBy}
//                                     onChange={(e) => setSortBy(e.target.value)}
//                                     className="mt-1 w-full bg-white/5 text-white border border-white/15 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-300"
//                                 >
//                                     <option value="startDateAsc" className="bg-[#0b2f6d] text-white">Start date ↑</option>
//                                     <option value="endDateDesc" className="bg-[#0b2f6d] text-white">End date ↓</option>
//                                     <option value="valueDesc" className="bg-[#0b2f6d] text-white">Contract value ↓</option>
//                                     <option value="valueAsc" className="bg-[#0b2f6d] text-white">Contract value ↑</option>
//                                 </select>
//                             </div>
//                         </div>
//                     </div>
...
//                     <div className="mt-5 flex items-center justify-between text-gray-200">
//                         <p className="text-sm">Page {page} of {totalPages}</p>
//                         <div className="flex gap-2">
//                             <button
//                                 onClick={() => setPage((p) => Math.max(1, p - 1))}
//                                 className="px-3 py-2 rounded-lg border border-white/20 hover:border-yellow-300 hover:text-yellow-300 transition"
//                             >
//                                 Prev
//                             </button>
//                             <button
...
//                     <button
//                         disabled={!selectedId}
//                         className={`md:col-span-2 w-full py-3 rounded-lg text-gray-900 font-bold text-sm tracking-wide transition-all active:scale-[0.98] ${selectedId ? "bg-yellow-300 hover:bg-yellow-400" : "bg-yellow-300/40 cursor-not-allowed"
//                             }`}
//                         onClick={() => {
//                             if (!selectedId) return;
//                             // Proceed with selected contract
//                             alert(`Proceeding with ${selectedId}`);
//                         }}
//                     >
//                         NEXT
//                     </button>
...
//                     <button
//                         className="w-full border border-gray-300 text-gray-200 hover:border-yellow-300 hover:text-yellow-300 py-3 rounded-lg font-medium text-sm tracking-wide transition-all active:scale-[0.98]"
//                         onClick={() => alert("Open Contract Dashboard")}
//                     >
//                         Contract Dashboard
//                     </button>
//                 </div>
...
//         <div
//             onClick={onSelect}
//             className={`bg-white/10 backdrop-blur-xl border ${selected ? "border-yellow-300" : "border-white/15"} 
//       rounded-xl shadow-xl p-5 transition-all cursor-pointer hover:scale-[1.01]`}
//         >
...
//                 </div>
//                 <button
//                     className="px-3 py-2 rounded-md text-xs font-semibold bg-white/20 text-white hover:bg-white/30 transition"
//                 >
//                     View summary
//                 </button>
//             </div>
...
//     return (
//         <div>
//             <p className="text-gray-300 text-[11px] uppercase tracking-wide">{label}</p>
...
//                 {[...Array(4)].map((_, i) => (
//                     <div key={i} className="bg-white/10 border border-white/15 rounded-xl p-5 animate-pulse">
...
//     return (
//         <div className="bg-white/10 border border-white/15 rounded-xl p-10 text-center">
//             <p className="text-white font-semibold">No projects found</p>
//             <p className="text-gray-300 text-sm mt-1">Try clearing filters or check your search terms.</p>
//             <button
//                 onClick={onReset}
//                 className="mt-4 px-4 py-2 rounded-lg border border-gray-300 text-gray-200 hover:border-yellow-300 hover:text-yellow-300 transition"
//             >
//                 Reset filters
//             </button>
//         </div>
...
//             <div className="relative w-full max-w-2xl bg-[#0b2f6d] text-white rounded-xl shadow-2xl border border-white/15 p-6">
...
//                     <button className="w-full bg-yellow-300 hover:bg-yellow-400 py-3 rounded-lg text-gray-900 font-bold text-sm tracking-wide transition-all active:scale-[0.98]">
//                         Create contract
//                     </button>
//                     <button className="w-full border border-gray-300 text-gray-200 hover:border-yellow-300 hover:text-yellow-300 py-3 rounded-lg font-medium text-sm tracking-wide transition-all active:scale-[0.98]">
//                         Download summary
//                     </button>
//                 </div>
//             </div>
